	...
	public static void main(String[] args) throws Exception {
		...
		if (params.get("retrievalAlgorithm").equals("UnrankedBoolean"))
			...
			if (k1 == null || k3 == null || b == null)
				System.out.println ("Error: please input 3 parameters for BM25.");
			else if (Double.parseDouble(k1) < 0)
				System.out.println ("Error: plaese input a positive number for k1.");
			else if (Double.parseDouble(k3) < 0)
				System.out.println ("Error: plaese input a positive number for k3.");
			else if (Double.parseDouble(b) < 0 || Double.parseDouble(b) > 1.0)
				System.out.println ("Error: parameter b should be in the range 0.0-1.0.");
			else {
			...
			if (mu == null || lambda == null)
				System.out.println ("Error: please input 2 parameters for Indri.");
			else if (Double.parseDouble(mu) < 0)
				System.out.println ("Error: plaese input a positive number for mu.");
			else if (Double.parseDouble(lambda) < 0 || Double.parseDouble(lambda) > 1.0)
				System.out.println ("Error: parameter lambda should be in the range 0.0-1.0.");
			else {
		...
		while ((curLine = queryReader.readLine()) != null) {
			...
			if (isBM25) {
				queryContent = "#SUM(" + curLine.substring(colonIndex + 1) + ")";
			}
			else if (isIndri)
				queryContent = "#AND(" + curLine.substring(colonIndex + 1) + ")";
			else
			...
			strBuffer = printResults(queryID, queryTree.evaluate(model));
    ...
    static Qryop parseQuery(String qString) throws IOException {
    	...
    	while (tokens.hasMoreTokens()) {
    		...
    		if (token.matches("[ ,(\t\n\r]")) {
    			...
    			// if statement: Ensure that the token is not ")" or other non-weight
    			...
    			int distance = Integer.parseInt(token.trim().substring(divIndex+1)); 
    		...
    		// the token specifies a particular field (e.g., apple.title).
    			if (isWop) { 
    				// To execute this block, if isWop is true, isWeight must be false,
    			...
    			if (tokenizedWord.length > 0) {
    				...
    				if (dotIndex != -1) { // has specific field
    					...
    					if (word == null || word.length == 0) { // Term is stopword
    						if (isWop) {
    							weight = weightStack.pop(); // remove the corresponding weight
    						}
    					}
    					else { 
    						if (isWop) {
    							weight = weightStack.pop();
    							((QryopSlW)currentOp).add(new QryopIlTerm(word[0], field));
    							((QryopSlW)currentOp).addWeight(weight);
    					...
    					if (isWop) {
    						weight = weightStack.pop();
    						((QryopSlW)currentOp).add(new QryopIlTerm(tokenizedWord[0]));
    						((QryopSlW)currentOp).addWeight(weight);
